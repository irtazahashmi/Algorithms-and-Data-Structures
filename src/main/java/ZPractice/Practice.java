...
public class Practice {
    ...
    public static void quickSort(int[] arr, int start, int end) {
        ...
        while(!stack.isEmpty()){
            ...
            // if pivotIndex between start and end
            if(start < pivotIndex - 1 && end > pivotIndex + 1){
                stack.push(start);
                //push left subarray index
                stack.push(pivotIndex - 1);
                //push right subarray index
                stack.push(pivotIndex + 1);
                stack.push(end);
                // pivot index start is greater the start
            } else if(start < pivotIndex - 1){
                stack.push(start);
                stack.push(pivotIndex - 1);
                // pivot index less than end
    ...
    public static void main(String[] args) {
        int arr[] = {14, 5, 1, 2, 15, 6, -1, 16, 4, 9, 8, 7};
