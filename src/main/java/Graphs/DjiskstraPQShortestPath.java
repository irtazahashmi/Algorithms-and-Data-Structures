...
public class DjiskstraPQShortestPath {

    public static List<VertexUG> shortestPath(UndirectedGraph g, VertexUG a, VertexUG b) {
        if (a == b) return new ArrayList<>();

        int[] distances = new int[g.getAllVertices().size()];
        VertexUG[] parents = new VertexUG[g.getAllVertices().size()]; // track parents to retrack path

        // Mark all vertices as distance infinity, except a
        for (VertexUG v : g.getAllVertices()) {
            if (v != a) {
                distances[v.getId()] = Integer.MAX_VALUE;
                parents[v.getId()] = null;
            }
        }
        ...
        while (!pq.isEmpty()) {
            ...
            for (VertexNumPair neighbour : currentVertex.getNeighbours()) {
                ...
                if (newDistance < distances[neighbourVertex.getId()]) {
                    ...
                    parents[neighbourVertex.getId()] = currentVertex;
                    pq.insertOrReplace(neighbourVertex, newDistance);
                }
            }
        }

        List<VertexUG> path = new ArrayList<>();
        VertexUG last = b;
        while(last != a) {
            if (last == null) return null;
            path.add(0, last);
            last = parents[last.getId()];
        }
        path.add(0, a);
    ...
    public static void main(String[] args) {
        // [<vertex: 0>, <vertex: 3>, <vertex: 2>, <vertex: 5>]
